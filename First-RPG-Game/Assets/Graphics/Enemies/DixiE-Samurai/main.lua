...
local initialise = function() 
    ...
    obj_shuriken_bleed:onStep(function(inst)
        ...
        if data.counter == 60 then
            ...
            data.counter = 0
            local attack_instance = data.parent:fire_direct(data.target, data.damage_coeff, 0, nil, nil, gm.constants.sNone, false)
            local attack_info = attack_instance.attack_info
            attack_info.knockback_kind = Attack_Info.KNOCKBACK_KIND.none
            attack_info:set_critical(false)
            attack_info:set_color(Color.from_rgb(100, 0, 0))
        end
    ...
    local net_counter_packet = Packet.new()
    net_counter_packet:onReceived(function (message, player)
        ...
        if actor then 
            ...
            if net_type == Net.TYPE.host then
                ...
                counter_msg:send_exclude(player)
            end
        end
    end)
    ...
    parry_callback = function(actor, attack_instance)
